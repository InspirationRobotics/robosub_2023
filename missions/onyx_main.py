"""
To create a sequential order of missions for Onyx to follow
"""

import time 
import signal # For handling signals

from auv.mission import cointoss_mission, buoy_mission, gate_mission, style_mission, dhd_approach_mission, surfacing_mission # Import the missions
from auv.utils import arm, disarm, deviceHelper # Functions for arming, disarming, loading
from auv.motion import robot_control # Robot_control.py contains the motion code
from auv.device.modems import modems_api # For intersub communication
import rospy

# Set the target to Abydos, heading of the gate to 60 degrees, DHD heading relative to the gates
target = "abydos"
gate_heading = 60 
dhdDir = -1 # 1 is +20 which is clockwise

# Initialize missions node
myNode = rospy.init_node("missions", anonymous=True)
time.sleep(30)

# Load sub configuration
config = deviceHelper.variables
# Load instance of RobotControl class (for motion)
rc = robot_control.RobotControl()

# Attempt to handshake with Graey (establish intersub communication)
try:
    modem = modems_api.Modem()
    modem.send_msg("graey handshake")
    fail_modem = False
except:
    fail_modem = True
    print("Failed to start modem, starting directly")

# Function for cleaning Modems and LEDs upon exit
def onExit(signum, frame):
    try:
        print("Exiting...")
        # Clean up the modems and LEDs
        modems_api.led.clean()
        if not fail_modem:
            modem.stop()

        myNode.stop()
        time.sleep(3)
        rospy.signal_shutdown("Rospy Exited")
        while not rospy.is_shutdown():
            pass
        print("\n\nCleanly Exited")
        exit(1)
    except:
        pass

signal.signal(signal.SIGINT, onExit) # If keyboard exception (Ctrl + C), exits loop/cleans up modems and LEDs
arm.arm() # Arm the sub

# Message to start cointoss mission
if not fail_modem:
    modem.send_msg("start onyx")
    modem.send_msg("coin toss")

# Run cointoss mission
coin_toss = cointoss_mission.CoinTossMission()
time.sleep(2)
coin_toss.run(gate_heading)
coin_toss.cleanup()

# Slight adjustments to the sub for after cointoss mission
time.sleep(2)
rc.forwardDist(6, 2)
time.sleep(2)
rc.lateralUni(-2,4)
time.sleep(1)
rc.setHeadingOld(gate_heading)

if not fail_modem:
    modem.send_msg("coin toss end")
    modem.send_msg("gate")

# Pass through the gate
gateMission = gate_mission.GateMission(target, **config)
gateMission.run()
gateMission.cleanup()
rc.forwardDist(2, 2)

if not fail_modem:
    modem.send_msg("gate end")
    modem.send_msg("style")

# Run the style mission
styleMission = style_mission.StyleMission()
styleMission.run(gate_heading-8)
styleMission.cleanup()


if not fail_modem:
    modem.send_msg("style end")
    modem.send_msg("buoy")

# Run the buoy approach
rc.forwardDist(5, 2)
rc.set_depth(1)
# time.sleep(4)
buoyMission = buoy_mission.BuoyMission(target)
buoyMission.run()
buoyMission.cleanup()

if not fail_modem:
    modem.send_msg("buoy end")
    modem.send_msg("heading to octagon")

# Goes above the buoy, then aligns with the Octagon
time.sleep(2)
rc.set_depth(1)
time.sleep(4)
rc.set_depth(1.3) # 0.5
time.sleep(2)
rc.forwardDist(3, 2)
time.sleep(2)
rc.setHeadingOld(gate_heading+(dhdDir*45)) # Guesstimating
time.sleep(2)

rc.forwardUni(2, 47) # Move forward at power 2 for 47 seconds

if not fail_modem:
    modem.send_msg("dhd approach")

# Run the DHD approach mission
dhd_app = dhd_approach_mission.DHDApproachMission(**config)
time.sleep(2)
dhd_app.run()
dhd_app.cleanup()

if not fail_modem:
    modem.send_msg("dhd approach end")
    modem.send_msg("surfacing")

# Run the surfacing mission
surfacing = surfacing_mission.SurfacingMission(**config)
time.sleep(2)
surfacing.run()
surfacing.cleanup()

if not fail_modem:
    modem.send_msg("surfacing end")

disarm.disarm() # Disarm the sub so nothing bad happens